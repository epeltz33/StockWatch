...
def register_callbacks(dash_app):
    @dash_app.callback(
        Output("watchlist-dropdown", "options"), Input("watchlist-interval", "n_intervals")
    )
    ...
    def update_watchlist(
        ...
        def toast(message, kind="info"):
            ...
            return {
                "message": message,
                "type": kind,
                "n": (ctx.triggered[0].get("value") if ctx.triggered else 0),
            }
        ...
        if trigger_type == "create-watchlist-button":
            if not new_watchlist_name or not new_watchlist_name.strip():
                return (
                    ...
                    no_update,
                    no_update_list,
                    toast("Watchlist name cannot be empty.", "danger"),
                )
            try:
                ...
                return (
                    update_watchlist_section(watchlist.id),
                    watchlist.id,
                    no_update_list,
                    toast(f'Created watchlist "{name}".', "success"),
                )
                ...
                return (
                    ...
                    no_update,
                    no_update_list,
                    toast("Could not create watchlist.", "danger"),
                )
        ...
        # Case 2: Add Stock Button Clicked
            ...
            button_index = next(
                (i for i, btn_id in enumerate(add_ids) if btn_id["index"] == stock_symbol), None
            )

            if (
                button_index is None
                or not add_clicks
                or button_index >= len(add_clicks)
                or not add_clicks[button_index]
            ):
                logger.info(
                    f"Add to watchlist ignored for {stock_symbol} - no valid click detected"
                )
                return no_update, no_update, no_update_list, no_update
            ...
            if not selected_watchlist_id:
                ...
                return (
                    ...
                    no_update,
                    no_update_list,
                    toast("Select a watchlist first.", "danger"),
                )
            ...
            try:
                ...
                if not watchlist:
                    return (
                        ...
                        no_update,
                        no_update_list,
                        toast("Watchlist not found.", "danger"),
                    )
                ...
                if not stock:
                    return (
                        ...
                        no_update,
                        no_update_list,
                        toast(f"Could not add {stock_symbol}.", "danger"),
                    )

                watchlist.stocks.append(stock)
                db.session.commit()
                logger.info(
                    f"Successfully added {stock_symbol} to watchlist {selected_watchlist_id}"
                )
                ...
                return (
                    ...
                    selected_watchlist_id,
                    reset_texts,
                    toast(f'Added {stock_symbol} to "{watchlist.name}".', "success"),
                )

            except Exception as e:
                db.session.rollback()
                logger.error(
                    f"Error adding stock {stock_symbol} to watchlist "
                    f"{selected_watchlist_id}: {str(e)}"
                )
                return (
                    no_update,
                    no_update,
            ...
            try:
                ...
                if watchlist and stock and stock in watchlist.stocks:
                    ...
                    db.session.commit()
                    logger.info(
                        f"Removed stock id {stock_id} from watchlist {selected_watchlist_id}"
                    )
                    return (
                        update_watchlist_section(selected_watchlist_id),
                        selected_watchlist_id,
                        no_update_list,
                        toast(f'Removed {stock.symbol} from "{watchlist.name}".', "success"),
                    )
                return (
                    update_watchlist_section(selected_watchlist_id),
                    selected_watchlist_id,
                ...
                return (
                    ...
                    no_update,
                    no_update_list,
                    toast("Could not remove stock.", "danger"),
                )
            ...
            try:
                ...
                if watchlist and watchlist.user_id == current_user.id:
                    ...
                    db.session.commit()
                    return (
                        update_watchlist_section(None),
                        None,
                        no_update_list,
                        toast(f'Deleted watchlist "{name}".', "success"),
                    )
                return (
                    update_watchlist_section(None),
                    None,
                ...
                return (
                    ...
                    no_update,
                    no_update_list,
                    toast("Could not delete watchlist.", "danger"),
                )
    ...
    def update_stock_data(
        watchlist_clicks, search_clicks, search_submit, watchlist_stock_ids, search_input
    ):
        ctx = callback_context
        ...
        if (
            triggered_prop.endswith(".n_clicks")
            and '"type":"load-watchlist-stock"' in triggered_prop
        ):
                        ...
                        html.Div(
                            [
                                html.Div(
                                    [
                                        html.Span(
                                            f"${stock_info['price']:,.2f}", className="price-lg"
                                        ),
                                        html.Div(
                                            build_change_readout(
                                                abs_change,
                                                pct_change,
                                                default_period,
    ...
    def update_chart_period(n_clicks_list, stored_data, intraday_data, symbol, btn_ids):
        ...
        btn_classes = [_period_btn_class(btn_id["index"] == active_period) for btn_id in btn_ids]
        readout = build_change_readout(abs_change, pct_change, active_period, since_date=since_date)
